...
export class ProfileComponent implements OnInit {
  ...
  updateUsernamePassword(form: NgForm) {
    ...
    const creds = [
      form.value.username,
      atob(this.auth.getCredentials()).split(":")[1]
    ];

    if (form.value.currentPass && form.value.newPass) {
      currentPasswordMatch =
        this.auth.generateBasicAuthCredentials(
          this.currentAuth.username.toLowerCase(),
          form.value.currentPass
    ...
    this.userService.updateUser(this.currentAuth).subscribe(
      data => {
        ...
        if (this.isEditByParent) {
              console.log('from parent')
          this.router.navigateByUrl(
            `user/${this.currentAuth.username}/profile/parent`
          );
        } else {
          this.auth.logout();
          ...
          this.router.navigateByUrl(
            `user/${this.currentAuth.username}/profile`
          );
        }
      },
      err => {
  ...
  updateUserDetails(form: NgForm) {
    ...
    this.user.userImageUrl = form.value.userImageUrl;
    ...
    this.userService.updateUserDetail(this.user).subscribe(
      data => {
        this.user = data;
        this.editUser = false;
        this.router.navigateByUrl(`user/${this.user.user.username}/profile`);
      },
      err => {
        this.router.navigateByUrl("not-found");
        console.error("Observer got an error: " + err);
        ...
        this.userService
          ...
          .subscribe(
            data => {
              ...
              if (
                this.router.url == "/user/" + this.user.user.username + "/profile/parent"
              ) {
                this.isEditByParent= true;
              }
